...
面向希望通过拍摄、反馈和练习进步的摄影爱好者，当前交付是 **Codex 技能 + 本地 HTML 点评报告 + 可追溯的校准记录**，不是独立 App，也不是打开网页就能上传评图的在线服务。

| 部分 | 怎么使用 | 当前边界 |
|---|---|---|
| 摄影点评技能 | 在 Codex 中附图，调用 `$photography-coach` | 由模型逐图观察、检索和判断，可能出错 |
| 视觉报告 | 打开每次生成的 `index.html`，对照点评、图片和练习 | 静态报告，不会自己调用模型、修图或接收上传 |
| 调整后图片 | 下载原图、仅调色、综合调整等实际提供的版本 | 先编辑并核对，再交给页面展示；不承诺每张都要裁切 |
| 校准与测试记录 | 查看本仓库的逐例证据与验证命令 | 解释发现过哪些问题，不代表已证明摄影准确率 |

截至 **2026-09-14**：本轮质量收尾已完成，156 项自动化测试通过，项目使用者已确认本轮示例页面。此确认只覆盖该示例，不是跨浏览器认证，也不等于正式发布质量门槛通过。
...
```text
请给我完整点评和可打开的视觉报告。先输出保留原取景的仅调色成片；如果裁切确实有帮助，再另给裁切＋调色版。说明每一步改善了什么、牺牲了什么，并附一张真正相关的优秀作品参考。
```

标题、EXIF、拍摄经过和“为什么按下快门”都不是必填项。没有上下文时，教练只依据画面中可见的关系判断，并把身份、地点、事件、意图和因果保持为未知。

使用前需要 Codex 能查看图片；核验外部作品需要网络访问，生成调整版需要可用且获授权的图像处理工具。技能本身不附带模型服务或修图服务。如果工具不可用，应明确返回建议和限制，不用原图或生成式近似图冒充已修好的照片。已有安装目录时不要覆盖其中的本地改动。

### 2. 可选：运行本地分析工具

以下命令在仓库目录内执行。报告渲染与知识结构校验使用 Python 标准库；图像完整性分析与 benchmark 物化需要 Pillow。
...
先完成点评和图像编辑，再按[报告字段与约束](references/visual-report.md)准备完整 JSON。文档中的 JSON 仅是字段示意，不能直接当作完整输入；实际需要八维评分和 6–9 条观察。
...
打开输出目录的 `index.html` 即可查看；转移报告时需要连同 `assets/` 一起保留。照片、点评 JSON 和生成页面应放在仓库外，仓库不附带本次用户照片或本机示例页面。

## 在页面里怎样看调整

先比较 **原图 / 仅调色**，保持取景不变，只看颜色和明暗；再看 **裁切＋调色**，单独判断裁切是否值得。没有提供的版本会隐藏，不为凑齐三个按钮强制修图。

- 默认展示仅调色版；缺少该版本时展示已有调整版，再没有则展示原图。
- 顶部“下载当前版本”跟随切换，下载完整文件；页面预览可以使用缩小副本。
- “在照片上看”将点评定位到专门的原图区域；裁切框标示保留范围，不叠在已裁切图片上。
- 八维分数各有理由，并保留完整性六检、具体参考作品和观看任务。
- “复制复盘说明”用于把下一次练习带回 Codex；自动复制不可用时提供手动复制文本。

只有原图时，页面会明确显示调整建议，不声称已经修好。渲染器只检查数据和文件，不判断调色是否好看、图片是否保真或参考是否相关；这些仍需教练逐图核对。
...
**156 项自动化测试通过，不等于摄影质量通过。** 本轮收窄了相应规则，尚未在另一批新图上验证修订后的行为。12 张是有目的的小规模查错，不发布通过率，不替代完整发布评测。逐例复核与当时的限制见 [`gpt6-final-review-20260914.md`](references/gpt6-final-review-20260914.md)；该记录中的 148 项测试和待页面验收是早期阶段状态，后续三版本实现增加了 8 项测试，项目使用者随后确认了本轮示例页面。

自动化脚本以 DOM 测试替身验证版本切换、下载链接和显示状态，不是真实浏览器测试。点评定位、裁切框和复制练习尚未有完整的点击回归覆盖；用户确认不替代这些后续测试，也不代表所有报告、屏幕或浏览器都已验证。
...
完整 benchmark 图片和本地工作包不提交到仓库。Git 保留来源、许可证、协议、部分历史答卷、评分及审查记录；本轮 24/12 张记录的原图和完整运行包仍在本地，因此公开 JSON 中的哈希与原句不等于外部读者拿到完整复现实验材料。不要把本机路径当成可访问的示例链接。

## 验证

在仓库目录运行测试和知识结构校验。完整测试需要 Pillow；三版本 JavaScript 检查还需要 Node.js（可用 `node --version` 检查），缺少 Node.js 时相关测试会跳过，不应把跳过当成通过：
