...
The 100-case holdout v3 is governed by `benchmark-method.md`. Its errors informed v1.5 and v1.6, so it is development evidence rather than independent certification. The later v4 was invalidated by materialized-image identity mismatches; v5 aggregate grades were withdrawn for unsupported bulk assignments and its cases subsequently informed calibration. A new source-disjoint evaluation is required; no prior set supplies current release evidence. Earlier sets remain development/audit archives. Passing source, masterwork, and rule counts does not change the stage by itself: all active benchmark images must materialize, the blind responses must be scored by band and genre, and the published result must include failures. Never search `benchmark-cases.jsonl` while critiquing a benchmark image; that file is an answer key, not coaching evidence.
