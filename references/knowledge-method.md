...
The 100-case holdout v3 is governed by `benchmark-method.md`. It failed release thresholds; its errors informed v1.5 and a development replay on those same images informed v1.6, so it is development evidence rather than independent certification. A newly sourced v4 is required. Earlier sets are retained as versioned or development archives. Passing source, masterwork, and rule counts does not change the stage by itself: all active benchmark images must materialize, the blind responses must be scored by band and genre, and the published result must include failures. Never search `benchmark-cases.jsonl` while critiquing a benchmark image; that file is an answer key, not coaching evidence.
