...
- The current repository answer key is the source-disjoint 100-case `blind_holdout_v3`. It failed its preregistered release gate and its errors informed v1.5, so it is development evidence for that revision and cannot certify it. A fresh source-disjoint `blind_holdout_v4` is required before release. Earlier sets remain archived for regression and leakage exclusion.
...
4. Save one frozen response at each declared `response_path`. Run `python3 scripts/validate_response.py --benchmark RESPONSE.md` per response or `python3 scripts/validate_blind_responses.py RUN_DIR` for the packet. Benchmark responses must record a completed integrity probe, three-lane relation coverage, an explicit information-loss gate, a four-part fact-boundary audit, and at least one compliant exact-work reference; the no-reference omission is not accepted when bundled verified cards are available. Do not alter a response after grading begins.
