...
4. Save one frozen response at each declared `response_path`. Run `python3 scripts/validate_response.py --benchmark RESPONSE.md` per response or `python3 scripts/validate_blind_responses.py RUN_DIR` for the packet. Benchmark responses must record a topology-first scene map, completed integrity probe, operation-specific crop/tilt/detail tests, three-lane relation coverage, two-relation protection gate, explicit information-loss gate, four-part fact-boundary audit, and at least one compliant exact-work reference; the no-reference omission is not accepted when bundled verified cards are available. Do not alter a response after grading begins.
