...
Before interpreting mood or story, complete the silent six-family gate in `references/integrity-preflight.md`: edge/crop pressure, shadow compression, highlight clipping, color excess/cast, roll/convergence with collateral crop, and detail damage. Compare the intended key region with adjacent detail at full useful resolution. If the viewing surface downsizes the file enough to hide the relevant pixels, inspect a non-destructive local region at useful scale or leave that pixel-level family unresolved; never use a known edit recipe as a substitute for seeing the finished pixels. Classify each family internally as `absent`, `plausible`, or `evidenced`; `evidenced` requires at least two independent observations. Record the strongest plausible objection and its falsifier even when the photograph basically works. Diagnose the visible state without inventing an editing history.
...
Never invent a flaw merely to sound useful. Never hide a key-region failure behind “style.” Natural overlap, complexity, multiple anchors, or an off-frame continuation is not a structural defect until the response names the specific relationship or information that becomes unreadable.
...
Expose the six-family result in `全画面看片地图` as the compact `完整性六检` table required by the response card. Every family must receive `通过`, `观察`, or `问题` plus a visible basis; do not mark a family `通过` merely because the key subject survives or another flaw feels more interesting. A localized loss can make a family `观察/问题` without making it the structural bottleneck; conversely, preserved detail in one region does not erase loss elsewhere. Keep the adaptive map below it so the critique still prioritizes relationships, meaning, and strengths rather than becoming a defect checklist.
...
Use operation-specific falsifiers rather than generic reassurance. For roll, first diagnose geometry: a reliable horizontal and a reliable vertical must both improve under the same signed counter-rotation; hills, roads, trees, curved facades, or one converging line are not enough. Then assess repair feasibility separately: note whether rotation would create collateral crop or require original-file margin. Worsened edge pressure can make the repair unavailable in the current file, but it does not falsify otherwise supported common roll. For crop pressure, test edge closure by comparing threatened-side room to the subject/action/repeated-module scale and to unrelated slack on the opposite side; literal edge contact is not required. For detail damage, check waxy smearing, halos or false microcontrast, and blocking or ringing, then compare high-frequency textures at similar apparent scale across two depth planes; do not use bokeh-versus-face softness as the sole comparison.
...
Give one prioritized reshoot action and distinguish what post-processing can and cannot repair. Add one 20–40 minute practice drill. Use observable landmarks or proportions instead of fake precision. When no measured preview exists, give a direction and visible stop condition before any approximate numeric starting point; do not present an eyeballed angle, exposure, crop, or slider value as a measurement.
...
Treat reference selection as a fit gate, not a search result. Declare whether it teaches correction of the leading problem, preservation of a named strength, or a contrasting boundary, following `reference-policy.md`. A candidate is eligible only when the response can localize that same mechanism in both images, a meaningful difference prevents recipe copying, and the user can transfer it into a concrete experiment. Similar subject matter, palette, mood, genre, or photographer fame is not enough. Give one reference by default; use two only when they teach genuinely different mechanisms.
...
In benchmark/audit runs, an exact-work reference is mandatory rather than optional. Apply the same fit gate and open-ended retrieval as ordinary use: either an admitted card or a newly verified exact work may qualify. Include the response-card comparison fields. If neither route yields a fit, record the omission honestly; that benchmark response remains structurally incomplete, rather than being filled with an irrelevant reference.
...
For a requested crop/color comparison, retain a full-frame color-only master separately from any cropped treatment. Supply it as `color_image` so the user can compare tone/color without a framing change. Create the cropped version from that same treatment when cropping is warranted; do not invent a crop just to fill a third tab. A file path or matching dimensions does not prove fidelity or visible improvement: inspect both images before supplying them. Follow the available image-editing tool's authorization requirements; the renderer only presents files and does not perform or authorize pixel editing.
...
Write report JSON and rendered files to a task-specific directory outside this skill repository. Never add a user photograph, treatment, report JSON, or rendered report to Git. Default to the inspected color-only version when present, otherwise to an inspected combined treatment; show the source beside it later and provide each master download. If no treatment passed source comparison, omit treatment controls instead of showing a simulated result.
