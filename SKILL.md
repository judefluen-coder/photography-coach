...
Record `完整性量化：已运行（触发：家庭[强/复核]；无信号时写“无阈值信号”）` or `完整性量化：不可用（原因）` immediately above the six-family rows. The trigger list must match the probe rather than memory. Never copy the probe's threshold label into the verdict. Resolve it with visible evidence: a raised cue plus a localized failure supports `观察/问题`; a raised cue plus a localized falsifier may still support `通过`. In audit mode, make each family test concrete: compare opposite frame edges, two dark planes, two bright planes, two color/material regions, two independent stability references, and two same-scale detail regions. Add a four-edge/center ledger, three named key-region relationships, a three-lane relation coverage check, and a two-candidate priority adjudication using information loss, relationship damage, affected area, reversibility, repair value, and protection cost. The three relation lanes are edge/center, space/action, and light-color/material; each must compare two localized things and name the resulting reading. For action photographs, establish approach/contact/suspension/aftermath from the visible body-hand-foot-tool interface before discussing speed, outcome, or background colour. The probe strength routes attention but does not settle priority: loss of a face, gesture, text, object identity, or material boundary may outrank a stronger but visibly organized color field. A confirmed broad or key-region information loss enters candidate A and cannot be silently demoted below a reversible taste preference; if another candidate wins, record the localized counter-evidence and protection cost. If any strong cue remains `观察/问题`, at least one such family must be the single public `首要`; otherwise name the localized counter-evidence that makes it pass. A strong axis cue means horizontal and vertical edge populations share a non-trivial signed deviation; treat it as a visible orientation decision (`观察` or `问题`), never `通过`. It may still be an intentional strength rather than a repair target.
